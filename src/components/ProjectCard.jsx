...
const ProjectCard = ({ image, liveLink, gitLink, name, description, tools }) => {
  return (
    <div className="group/img hover:border-purple-500 transition-transform hover:duration-200 hover:ease-in-out hover:scale-103 duration-200 ease-in hover:shadow-purple-400 hover:shadow-sm rounded-2xl shadow-lg w-[332px] h-[544px] gap-4 bg-gray-950 flex flex-col shadow-violet-950 p-4 overflow-hidden">
      {/* Image */}
      <img className="h-[196px] rounded-lg transition-transform object-cover object-top group-hover/img:scale-105 hover:duration-200 hover:ease-in" src={image} alt={name} />
      ...
      <div className="flex gap-2 items-center justify-center mt-4">
        ...
        <a href={liveLink} target="_blank" rel="noreferrer">
          <button className="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-purple-600 to-blue-500 group-hover:from-purple-600 group-hover:to-blue-500 hover:text-white dark:text-white focus:ring-4 focus:outline-none hover:cursor-pointer">
            <span className="relative px-5 py-2.5 transition-all ease-in duration-75 text-white bg-gray-900 rounded-md group-hover:bg-transparent group-hover:dark:bg-transparent">
              View Live
            </span>
          </button>
        </a>
